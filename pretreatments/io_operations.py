...
def read_user_list():
    ...
    return  pd.read_csv(user_listFile, sep='\t', names=user_colnames, header=1, dtype=user_datatype, encoding = 'latin-1')
    

def read_service_list():
    
    return pd.read_csv(service_listFile, sep='\t', names=service_colnames, header=1, dtype=service_datatype, encoding = 'latin-1')
